...
int loramode;
int LoRa_begin(LoRa_ctl *modem){loramode = SLEEP_MODE;return 0;}
void LoRa_send(LoRa_ctl *modem){loramode = TX_MODE;}//After sending auto switches to standby mode.
void LoRa_receive(LoRa_ctl *modem){loramode = RXCONT_MODE;}//Continuous mode. You have to manually stop receiving if you want to. For example you can do it in callback right after catching packet.
void LoRa_calculate_packet_t(LoRa_ctl *modem){}
_Bool LoRa_check_conn(LoRa_ctl *modem){return true;}
void LoRa_end(LoRa_ctl *modem){}
void LoRa_stop_receive(LoRa_ctl *modem){}
void LoRa_sleep(LoRa_ctl *modem){loramode = SLEEP_MODE;}
unsigned char LoRa_get_op_mode(LoRa_ctl *modem){return loramode;}// Compare with SLEEP_MODE, RXCONT_MODE, SLEEP_MODE, STDBY_MODE
