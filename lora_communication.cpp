...
void httplib_handle_request(struct http_request_s* request) {
...
void *httpThread(void *d){
    struct http_server_s* server = http_server_init(8008, httplib_handle_request);
...
int main()
{
    ...
    pthread_create(&http_thread, NULL, httpThread, NULL);
